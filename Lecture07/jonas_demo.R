...
x2 <- as.data.frame(mvrnorm(n = 200,
                            mu = c(0, -4),
                    Sigma = matrix(c(12, 3, 3, 3), nrow = 2)))

x3 <- as.data.frame(mvrnorm(n = 200,
                            mu = c(3, 12),
...
kmeans.res <-  kmeans(df, 3, nstart = 25)
dbscan.res <- dbscan(df, eps = 0.3, minPts = 4)

clustering <- function(df, method.res){
  sample_df <- df %>% mutate(method.cluster = method.res$cluster)
  print(sample_df)
  print(sample_df$method.cluster)
  sc <- ggplot(sample_df, aes(V1, V2, color = method.cluster)) + 
    geom_point()
  sc
}

clustering(df, dbscan.res)
